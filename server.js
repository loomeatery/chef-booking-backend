...
// -------------------- FINAL ADMIN UI – NO CRASH, EVERYTHING WORKS --------------------
...
<div class="flex">
  <div class="col">
    <h2>Bookings</h2>
    <div id="bookings">Enter key and click Save</div>
...
(() => {
  ...
  async function load() {
    ...
    // Bookings
    let html = '';
    const bRes = await fetch('/__admin/list-bookings?' + params, { headers });
    ...
    // Gift Cards
    const gRes = await fetch('/api/admin/giftcards', { headers });
    ...
    // Blackouts
    const blkRes = await fetch('/__admin/list-blackouts?' + params, { headers });
