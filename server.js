  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (md.type === "gift_card") {
        ...
        const code = `CHRIS-GIFT-${randomPart}`;
        ...
        const prettyAmount = (Number(md.amount_cents) / 100).toFixed(2);
        const basketText = md.with_basket === "true" ? " + Fresh-Baked Gift Basket" : "";

        await sendEmail({
          to: md.buyer_email,
          subject: `Gift Card Confirmed – $${prettyAmount}${basketText}`,
          html: `
            <h2>Thank you, ${md.buyer_name.split(" ")[0] || "there"}!</h2>
            <p>Your purchase of a <strong>$${prettyAmount}${basketText}</strong> gift card is confirmed.</p>
            <p>Chef Chris will personally prepare and email the beautiful PDF gift card to <strong>${md.recipient_name}</strong> within 24 hours.</p>
            <p><strong>Redeem code (for your records):</strong><br>
            <span style="font-size:20px;color:#bfa87c;font-weight:bold">${code}</span></p>
            <hr>
            <p>Questions? Just reply to this email.</p>
            <p>Warmly,<br>Chef Chris</p>
          `
        });

        console.log(`✅ Gift card ${code} created and buyer notified`);
        return res.json({ received: true });
      }

      // ==================== ORIGINAL BOOKING FLOW (UNCHANGED) ====================
      ...
      const logoBlock = logoUrl ? `<img src="${logoUrl}" alt="Chef Chris" width="48" height="48" style="border-radius:50%;display:block;margin-bottom:8px"/>` : "";
      const html = `
...
app.post("/api/quote", (req, res) => {
  try {
    ...
    const PKG = {
      ...
      dinner2: { perPerson: 250, depositPct: 0.30 },
    };
...
app.post("/api/book", async (req, res) => {
  try {
    ...
    const bartenderFeeCents = bartender ? 300 * 100 : 0;
    const tablescapeFeeCents = tablescape ? (15 * guests) * 100 : 0;
...
app.post("/api/giftcards/create-checkout", express.json(), async (req, res) => {
  try {
    ...
    if (!amount || amount < 25) return res.status(400).json({error:"Minimum $25"});
    if (!buyer_name || !buyer_email || !recipient_name || !recipient_email) return res.status(400).json({error:"Missing required fields"});
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        ...
        ...(basket ? [{ price_data: { currency: "usd", product_data: { name: "Fresh-Baked Gift Basket (+$125)" }, unit_amount: 12500 }, quantity: 1 }] : [])
      ],
...
<html lang="en"><head><meta charset="utf-8"/><title>Gift Card Purchased!</title>
<style>
  body{font-family:system-ui;background:#0f0f0f;color:#fff;text-align:center;padding:80px;line-height:1.6}
  h1{font-size:42px;margin:0 0 16px}
  a{color:#bfa87c;text-decoration:none;font-weight:600}
</style></head><body>
<h1>Thank You! 🎁</h1>
<p>Your gift card purchase is complete.</p>
<p>You’ll receive a confirmation email shortly.</p>
<p>Chef Chris will prepare and personally send the PDF gift card to the recipient within 24 hours.</p>
<br><a href="/">← Back to privatechefchristopherlamagna.com</a>
...
// (all your original admin routes are here - exactly as you had them)

// ----------------- Admin UI (fixed template literals) -----------------
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        ...
        price_data: {
          currency: "usd",
          unit_amount: Number(ev.price || 11500),
          product_data: {
      ...
      custom_fields: [
        { key: "dietary", label: { type: "custom", custom: "Dietary Restrictions or Allergies" }, type: "text", optional: true },
        { key: "guest_names", label: { type: "custom", custom: "Guests Name(s)" }, type: "text", optional: true },
        { key: "referral", label: { type: "custom", custom: "How Did You Hear About Us?" }, type: "text", optional: true }
      ],
      metadata: {
        event_id: id,
        quantity: String(qty),
        event_date: (ev.dateISO || "").slice(0, 10),
